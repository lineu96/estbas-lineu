...
\begin{frame}{Estatística descritiva ou exploratória}
\protect\hypertarget{estatuxedstica-descritiva-ou-exploratuxf3ria}{}
\begin{itemize}
\item
  Técnicas destinadas a descrever e resumir dados a fim de tirar
  conclusões a respeito de características de interesse.
\item
  Compreende a etapa de consistência dos dados, isto é, verificar se os
  dados coletados são condizentes com a realidade.
\item
  Fornece técnicas para resumos numéricos, medidas descritivas e
  visualizações gráficas.
\item
  Permite compreender preliminarmente o comportamento das variáveis
  individualmente e as interações entre elas.
\end{itemize}
\end{frame}

\begin{frame}{Probabilidade}
\protect\hypertarget{probabilidade}{}
\begin{itemize}
\tightlist
\item
...
\item
  Teoria matemática utilizada para se estudar a incerteza natural de
  fenômenos de caráter aleatório.
\end{itemize}
\end{frame}

\begin{frame}{Inferência estatística}
\protect\hypertarget{inferuxeancia-estatuxedstica}{}
\begin{itemize}
\tightlist
\item
  Métodos para estimação de quantidades populacionais por meio da
  amostra.
\item
  Métodos para formular e testar hipóteses (avaliar se existe evidência
  nos dados a favor de uma hipótese).
\item
  Métodos para extrapolar para a população resultados obtidos na
  amostra.
\end{itemize}
\end{frame}
...
\begin{itemize}
\item
  Pense no uso de medicamentos um medicamento não é aprovado se não
  houver um estudo que envolva um procedimento de amostragem coerente,
  uma análise estatística robusta que mostre evidência de eficácia do
  medicamento na população de interesse.
\item
  Outro exemplo é o uso da Estatística nas políticas públicas:
  praticamente todos os governos possuem órgãos especializados em
  Estatística.
\end{itemize}
\end{frame}
